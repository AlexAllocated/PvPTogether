...
- `/pt debug` and `/pt dump` open the common console. The console uses native WoW dialog artwork and button textures on addon-owned regions. All consumers use the same category popup, fuzzy/quoted search, tail-following scroll behavior, copy/clear controls, and test/diagnostics/log/reload buttons. `/pt dump CATEGORY` selects a category; `/pt dump clear` clears history without resetting nameplate counters.
